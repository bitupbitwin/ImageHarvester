...
- **多维筛选**：
  - 宽度 / 高度 **双滑块区间筛选**（步长 10，左手柄定下限、右手柄定上限，量程随页面最大图自动扩展；右手柄贴最右时不设上限）
  - 图片格式 / URL 关键字 / 隐藏失效与超小图
- **排序**：尺寸大→小、小→大、页面顺序、文件名
- **批量操作**：全选 / 反选 / 单击选择，一键下载选中项
- **打包下载 (ZIP)**：勾选后把选中图片打成一个 ZIP 文件（纯前端生成，支持中文文件名、自动重名去重）
- **自定义子文件夹**：逐张下载时存到 `下载目录/你的子文件夹/`；打包模式下作为压缩包文件名
...
5. 逐张下载：可填子文件夹名 → 点击 **下载选中**
   打包下载：勾选 **打包下载 (ZIP)** → 点击 **下载选中**，得到一个 ZIP 压缩包
...
node test_e2e.mjs         # 加载真实扩展，验证渲染/滑块筛选/选择/逐张下载/ZIP 打包全流程
